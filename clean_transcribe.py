...
class CleanTranscriber:
    ...
    def transcribe(self, audio_file, model="small", language="en", mode="production"):
        ...
        """
        print("🎵 Clean Transcription Starting...")
...
Examples:
  ...
  python clean_transcribe.py --list --dev                 # List dev transcriptions
    ...
    parser.add_argument("--dev", action="store_true", help="Development mode (simpler filenames)")
    parser.add_argument("--list", action="store_true", help="List existing transcriptions")
    ...
    if args.list:
        ...
        mode = "development" if args.dev else "production"
        result = transcriber.transcribe(args.audio_file, model=args.model, language=args.language, mode=mode)
